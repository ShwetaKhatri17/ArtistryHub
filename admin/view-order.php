...
// Check if the tracking number is provided and not empty
if(isset($_GET['t']) && !empty(trim($_GET['t']))) {
    $tracking_no = mysqli_real_escape_string($con, trim($_GET['t'])); // Prevent SQL injection

    // Assuming checkTrackingNoValid is a function that returns mysqli_result object on success or false on failure
    $orderData = checkTrackingNoValid($tracking_no);

    // Check if any order data was returned
    if($orderData && mysqli_num_rows($orderData) > 0) {
        $data = mysqli_fetch_assoc($orderData); // Use mysqli_fetch_assoc for associative array
    } else {
        // No order found with the provided tracking number
        echo "<h4>No order found for the provided tracking number.</h4>";
        include('includes/footer.php'); // Ensure the footer is included
        die();
    }
} else {
    // Tracking number not provided or empty
    echo "<h4>Tracking number not provided or invalid.</h4>";
    include('includes/footer.php'); // Ensure the footer is included
    die();
}

// Proceed with displaying the order details
// Make sure your HTML structure starts from here
?>
